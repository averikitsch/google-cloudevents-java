...
public class SourceProvenance {
    ...
    /**
     * Hash(es) of the build source, which can be used to verify that the original source
     * integrity was maintained in the build. Note that `FileHashes` will only be populated if
     * `BuildOptions` has requested a `SourceProvenanceHash`. The keys to this map are file
     * paths used as build source and the values contain the hash values for those files. If the
     * build source came in a single package such as a gzipped tarfile (`.tar.gz`), the
